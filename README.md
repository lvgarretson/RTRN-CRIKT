# RTRN-CRIKT
<b> By Team 9  &#129431;<br><b>

## Quick App Overview:


<b>Project Description:</b> An app that monitors gym air quality (AQI) and provides gym users and managers with actionable steps to improve the quality of the gym to improve workout performance and healthiness at the benefit of the gym. 
<br><b>Features:</b> Simple UI that visually plots and graphs data, then gives users actionable steps to improve workout performance or gym experience. 

<br><b>Intended Audience:</b> Gym managers and gym users

<br><b>Data/Metrics:</b> <li>AQI (Air Quality Index)</li><li>Temperature</li><li>Humidity</li><li>CO2</li><li>TVOC (Total Volatile Organic Compounds)</li>

<br><b>Manual:</b> TBD
<br><b>Limitations:</b> Currently only uses sample data. The sensors assume the data is collected in 1 minute intervals. (Other limitations TBD as testing continues)

---

## How to start the App:

<b>Link to Matlab:</b> [![Open in MATLAB Online](https://www.mathworks.com/images/responsive/global/open-in-matlab-online.svg)](https://matlab.mathworks.com/open/github/v1?repo=lvgarretson/RTRN-CRIKT/Daniel's-Branch)<br>
<b>(We need to finish the UI to write this part.) </b>

<b> <ins>App Tutorial:</ins> (we should add a video of it working here)</b>

---

## How the App Works:

<b>The app first processes the data from the sensors. It does this by first deleting the NAN values and then smoothering data by using the mean of all of the values in the same timespan. Then this values are presented as graph where each different color represents a different area inside of the gym. </b>

<b>We should add more information about the UI and additional things done in this part.</b>

---
